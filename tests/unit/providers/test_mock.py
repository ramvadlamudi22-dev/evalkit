...
class TestMockProvider:
    async def test_inline_mapping_dispatches_by_case_and_model(self) -> None:
        provider = MockProvider(
            responses={("c1", "m1"): "RESPONSE-A", ("c1", "m2"): "RESPONSE-B"},
            latency_ms=0,
        )
        a = await provider.complete(_request("m1", content="x", case_id="c1"), timeout_s=1)
        b = await provider.complete(_request("m2", content="x", case_id="c1"), timeout_s=1)
        assert a.text == "RESPONSE-A"
        assert b.text == "RESPONSE-B"
        assert a.raw == {"mock": True}

    async def test_falls_back_to_echoing_user_message(self) -> None:
        provider = MockProvider(latency_ms=0)
        response = await provider.complete(_request("any", content="ping"), timeout_s=1)
        assert response.text == "ping"

    async def test_loads_fixture_jsonl(self, tmp_path: Path) -> None:
        ...
        response = await provider.complete(
            _request("y", content="ignored", case_id="x"), timeout_s=1
        )
